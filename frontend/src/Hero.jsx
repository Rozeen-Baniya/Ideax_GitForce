...
export const LaserFlow = ({
    ...
    useEffect(() => {
        ...
        // Initial polling to ensure we catch the correct size if layout shifts happen without RO firing
        // or if the initial size was (1,1)
        const initInterval = setInterval(() => setSizeNow(true), 200);
        const initTimer = setTimeout(() => clearInterval(initInterval), 2000);
        ...
        const animate = () => {
            ...
            // Force render if we haven't faded in yet, even if not technically "in view" or paused
            // This prevents the "black screen until interaction" issue if the observer fires late
            const needsInitialFade = !hasFadedRef.current;
            if (!needsInitialFade && (pausedRef.current || !inViewRef.current)) return;
